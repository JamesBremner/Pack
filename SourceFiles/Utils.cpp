...
void Utils::displayHelp()
{
	...
	cout << "use: packit4me [options] " << "example: packit4me -b 0:ft:5x5x5 -i 0:ft:0:1x1x1 -s b -o result" << endl << endl;
	cout << "\t-b\t- bin or bins size (format: {id}::{dim_unit}:{size1}x{size2}x{size3}). comma dil multiple" << endl;
	cout << "\t-i\t- item or items size (format: {id}:{dim_unit}:{constraints}:{size1}x{size2}x{size3}) comma dil multiple" << endl;
